...
def pixels_matching_calculated_coordinates(auto_complete,floor):
    ...
    """
    full_df = pd.read_csv(os.path.join(os.getcwd(),VISUALS,f'{floor}',f'{floor}_pixels_coordinates.csv'))
    #Convert the float numbers into interger
    full_df['y_axis']=full_df['y_axis'].astype(int)
    full_df['x_axis']=full_df['x_axis'].astype(int)
    ...
    for index, row in full_df[full_df['room'].isnull()].iterrows():
        ...
        cv2.rectangle(image, ((int(row['x_axis']),int(row['y_axis'])+3)),((int(row['x_axis'])+28),int(row['y_axis'])-8), color=(255, 255, 255), thickness =-1)
