...
def get_value_coordinate_vertical(df,y_to_q_index):
    '''
    Quick and dirty fix
    Helper function to retrieve corresponding Coordinate number to corresponding coordinates pixels. Vertical version.
    '''
    return y_to_q_index.get((df['x_axis_angle_floor'], df['y_axis_angle_floor']))

def south_building_floor_pixels_coordinates(floor,letter_horizontal,letter_vertical,floor_points,floor_points_rotated_back,angle_floor_rotation,cXorient, cYorient,h, w,auto_complete,tolerance_x_floor_auto,tolerance_y_floor_auto,y_threshold_floor_auto,a_index_floor_auto):
    '''
    ...
    '''
#Calling BLACKED from the contour detection block
    ...
    tolerance_y = tolerance_y   # Tolerance for Q
    ...
    for x, y in rotation:
        a_idx = x_to_a_index[x, y]
        q_idx = y_to_q_index[x, y]
        label = f"{letter_horizontal}{a_idx:02d}{letter_vertical}{q_idx:02d}"
    ...
    if not auto_complete == 'yes':
        ...
        #cv2.imwrite('Labels_frist_part.png',image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    else:
        print('south_building_floor_pixels_coordinates -Status: Done')
    ...
    floor_points_df['coordinate_vertical'] = floor_points_df.apply(lambda row: get_value_coordinate_vertical(row, y_to_q_index), axis=1)
